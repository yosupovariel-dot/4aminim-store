...
- **תמונות**: ניתן להעלות תמונות אמיתיות לכל סט ישירות דרך עמוד
  `/admin/sets` (כמה תמונות לכל סט, עם אפשרות לסמן תמונה ראשית ולמחוק).
  ההעלאות נשמרות ב-Vercel Blob (Storage → Blob בפרויקט ב-Vercel) — צריך
  שה-store יהיה קיים ומחובר (ומוגדר **Public**, עם `BLOB_READ_WRITE_TOKEN`)
  לפני שההעלאה תעבוד. סטים ללא תמונה מציגים placeholder
  (`public/images/placeholder-set.svg`).
